...
- [chvm](https://github.com/crewhaus/chvm) — the CrewHaus version manager, and **the recommended way to install `crewhaus`**: `npm install -g @crewhaus/chvm`, then `chvm use latest`. Pinned side-by-side installs, instant shim-based switching, plus `system` and factory-checkout (`local`) targets. Runs on macOS, Linux, and Windows. Homebrew, Scoop, winget, apt, and npm remain supported alternatives
