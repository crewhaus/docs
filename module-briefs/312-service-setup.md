...
- **`agentmail.ts`** — inbox find-or-create, idempotent on a derived `client_id` (the API stores the resource against that id and replays it on reuse, so there is no list-then-create race), plus inbox-scoped API keys. Provider errors carry their own `fix` prose, which this module prefers over anything it would invent.
