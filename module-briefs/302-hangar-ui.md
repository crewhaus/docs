...
M1 was read-only over harness state. M2 made the console a driver — start, stop, restart, drain, watch a run live, poke scheduler lanes, settle approvals and review items — with every action showing the exact CLI command it runs. M3 added eight harness tabs and three fleet screens over the 178-route contract. M5 added the Advisor and Library curation: one more harness tab (`#/h/<id>/advisor`, right after Overview — the strip order is the triage order) and one more fleet screen (`#/advisor`, every harness's rollup, worst first). Every feed item carries a tooltip explaining why it matters and a quick action with its CLI twin; acting takes an optional comment, dismissing REQUIRES a reason, and zero open items renders as a first-class "running optimally" state, never an empty screen. In the Library, harness rows carry a `hidden` flag (the row editor's Visibility checkbox), hidden rows fold out of every view except the **Hidden** rail bucket, and a **Find harnesses…** panel lists unregistered candidates with per-candidate Add buttons — the blanket Scan stays the labeled bulk fallback.
...
- **`assets/js/routes.js`** — the grouped route contract as **pure data**: the 178 M3 routes plus the `advisor` group, each key tagged with its group. `api.js` builds one client wrapper per key from that map instead of hand-writing each; a hand-written name wins a collision, so the M1/M2 verbs keep their bespoke wrappers. The same table is what the server's contract test asserts set-equality against.
- **The hash router and its screens** — `#/` (Library), the fleet screens `#/{runs, approvals, review, activity, credentials, feedback, thredz, advisor}`, `#/h/<id>` and its tabs `{advisor, spec, runs[/<runId>], schedulers, sessions, evals, memory, data, feedback, costs, creds, channels, security, thredz, deploy, inspect, dev}`. The eight M3 tabs capture their trailing segments generically, so a sub-screen inside one never needs a new route case.
