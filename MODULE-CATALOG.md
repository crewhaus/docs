...
| ✅ `harness-registry` | The machine-wide harness registry — one `harnesses.json` (format v2) under `CREWHAUS_REGISTRY_ROOT` (default `~/.crewhaus`) with stable `hrn_` ids, user-managed groups/tags/pins, scan roots, and watchme interop. Atomic 0600 tmp+rename writes behind a fingerprint-checked read-merge-write loop; vanished directories are stamped `missingSince`, never pruned. Backs `crewhaus harness` and the Hangar console. | All | T1, T3 | `watchme-store` |
...
| ✅ `hangar-server` | The Hangar manager's local HTTP server: token-gated loopback API over the registry — fleet rows with cached rollups, per-harness detail, registry CRUD, supervised process control, a live SSE run feed, a `crewhaus.control.v1` proxy, approvals/review inboxes, and the M3 detail surface (178 routes, eleven groups). Bearer on every `/api` route, no cookies, single-use `/boot/<nonce>` token handoff. | All | T1, T3, T8 | `harness-registry`, `harness-inventory`, `harness-supervisor`, `preflight`, `gateway-protocol`, the stores it reads |
| ✅ `hangar-ui` | The Hangar console's static UI — one shell, one CSS file, hand-written browser ES modules embedded as text for the single-binary CLI. Zero build step, no framework, no npm dependency; the 178-route contract lives as pure data and generates the client wrappers. | All | T1 | — |
