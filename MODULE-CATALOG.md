...
| ✅ `target-eval-bundle` | Codegen for `eval` target — dataset loader, runner, graders, report. Also owns the `compile --with-eval-harness` **bridge**: `createBridgeInvoker` drives the source shape's actual compiled runtime (workflow step sequence, graph to `run_done`, one crew turn, pipeline retrieve+answer, channel `runTurn` loopback, managed `runOneTurn` under an isolated tenant), `guardHistorySamples` rejects history-carrying samples against non-chat-capable shapes at dataset load, and `emitSourceBundleWithEvalEntry` adds the `runForEval` export / `eval-entry.ts` the bridge imports. A plain compile stays byte-for-byte identical. | EVAL | T1, T5 | `compiler-core`, `eval-runner`, `dataset-registry`, `grader-registry`, the multi-stage target emitters, `target-channel-bot`, `tenancy` |
...
| ✅ `harness-registry` | The machine-wide harness registry — one `harnesses.json` (format v2) under `CREWHAUS_REGISTRY_ROOT` (default `~/.crewhaus`) with stable `hrn_` ids, user-managed groups/tags/pins, a first-class `hidden` visibility flag (`setHidden` — hidden entries stay registered; upsert refreshes never clobber it), scan roots, and watchme interop. Atomic 0600 tmp+rename writes behind a fingerprint-checked read-merge-write loop; vanished directories are stamped `missingSince`, never pruned. Backs `crewhaus harness` and the Hangar console. | All | T1, T3 | `watchme-store` |
| ✅ `harness-inventory` | Cross-harness discovery (depth-capped walk for `crewhaus.yaml`), the fleet inventory + health rollup, exit-code description, and the seam-injected bulk runner — the `crewhaus fleet` core, lifted out of the CLI byte-identically. | All | T1, T3 | `errors`, `feedback-distill`, `spec-registry` |
| ✅ `preflight` | Typed pre-spawn health checks over seven areas (spec → credentials → channels → mcp → ports → bundle → durability) with `info`/`warn`/`blocking` levels and stable ids. Every core function takes an injected `env`, so the same checks serve the CLI, tests, and a manager evaluating the *merged spawn* env. | All | T1, T3 | `errors`, `spec`, `model-router`, `mcp-host` |
| ✅ `harness-supervisor` | The harness process layer — spawn contracts per run class, the preflight gate (channel secrets unforceable), the supervision state machine with exit classification and a 5-per-10-minutes restart window, byte-exact log capture + `TraceEvent` pump, adoption, and the job queue. All state is harness-local under `<harness>/.crewhaus/run/`. | All | T1, T3, T7 | `errors`, `harness-inventory`, `preflight` |
| ✅ `hangar-server` | The Hangar manager's local HTTP server: token-gated loopback API over the registry — fleet rows with cached rollups, per-harness detail, registry CRUD, supervised process control, a live SSE run feed, a `crewhaus.control.v1` proxy, approvals/review inboxes, the M3 detail surface (189 routes, twelve groups), and the Advisor (group `advisor` on the same dispatch table) — a severity-ranked feed derived from signals other panels already read, act/dismiss/reopen decision ledgers, trend, reports, and an issue inbox that queues only the closed `ADVISOR_JOB_ARGV` vocabulary — plus library curation (`PUT /api/h/:id/visibility`, `GET /api/registry/discover`). Bearer on every `/api` route, no cookies, single-use `/boot/<nonce>` token handoff. | All | T1, T3, T8 | `harness-registry`, `harness-inventory`, `harness-supervisor`, `preflight`, `gateway-protocol`, the stores it reads |
| ✅ `hangar-ui` | The Hangar console's static UI — one shell, one CSS file, hand-written browser ES modules embedded as text for the single-binary CLI. Zero build step, no framework, no npm dependency; the grouped route contract (the 178 M3 routes plus the `advisor` group) lives as pure data and generates the client wrappers. Draws the Advisor tab and `#/advisor` fleet board, and the curated Library — hidden rows fold into a Hidden rail bucket, and a "Find harnesses…" panel lists discovery candidates without registering them. | All | T1 | — |
| ✅ `qr-code` | A QR Code encoder and terminal renderer with no dependencies — byte mode, versions 1–40, all four EC levels, the spec's mask evaluation and Reed–Solomon over GF(256). Renders with half-block characters so modules come out square in a terminal, and pins its own contrast with an explicit black-on-white SGR pair so the symbol scans on any theme. Exists for `crewhaus hangar --lan`, which prints a code to scan with a phone. | All | T1 | — |
...
| ✅ `eval-dataset` | JSONL / CSV / YAML / HTTP dataset loaders; lazy AsyncIterable. An optional per-sample `history` (`[{role, content}]`) is seeded verbatim into the transcript before the one graded turn; the CSV `metadata` column parses as JSON; `.jsonl`/`.ndjson` stream incrementally both locally and over HTTP (CSV and YAML still buffer). | EVAL | T1 | `errors`, `yaml`, `zod` |
| ✅ `eval-grader` | exact_match / contains / **expected_contains** / regex / json_path / schema / tool_call_sequence + composers; top-level `combine: all\|any\|weighted` with `passing_threshold` and per-grader `weight`; strict configs, so a typo'd key is a loud parse error. | EVAL | T1, T9 | `eval-dataset` |
| ✅ `eval-judge` | LLM-as-judge with sentinel-token prompt-injection defense. Abstention (`abstain` + `confidence` on `submit_score`); decoding `temperature` pinned to **0** by default; `repeats: k` median panels; multi-model `judges:` panels graded by median score and strict-majority pass, with vote entropy flagging a split verdict for review; categorical rubrics via a forced `submit_label`; `target: transcript` digest judging; pairwise `submit_comparison` for `eval-report diff --pairwise`; per-criterion scores reported; judge-token metering. | EVAL | T1, T8 | `eval-grader`, `runtime-core` |
| ✅ `eval-runner` | Per-sample fresh RunContext; concurrency; SIGINT-flushes results.json. Owns slices, abstention/needs-review/canary bucketing, Wilson + Student-*t* confidence intervals, flake detection, per-sample timeouts and the run budget meter, tool record/replay, resume, and the runner-local `calibration.*` / `consistency.*` grader packs. | EVAL | T3, T7 | `eval-dataset`, `eval-grader`, `eval-judge`, `runtime-core` |
| ✅ `eval-report` | HTML + JSON renderer; aggregates; diff mode with paired significance testing (sign-flip permutation, seeded bootstrap CI, deterministic PRNG — never `Math.random`), per-slice deltas and opt-in pairwise judging; plus `trends` (per-lineage charts from the run index, inline SVG, fully offline) and `export` (one row per run × sample × grader, CSV or JSONL). | EVAL | T1 | `eval-runner` (types only) |
| ✅ `dataset-registry` | Versioned + split-aware dataset storage; split-leak guard. Adds `verifySplitHashes` + `overallDatasetHash` (the `datasets verify` integrity gate), `PutOptions.allowOverwrite` (an existing version is refused without it), an additive `releases` / `ReleaseEntry` history for holdout spend, and a hard invariant refusing a `synthetic` sample that carries an `expected_output`. | EVAL | T1, T8, T9 | `eval-dataset`, `infra-utils` |
| ✅ `grader-registry` | Global registry of named graders + plugin discovery (`<cwd>/.crewhaus/graders`, discovered last so a plugin overrides any pack entry). A `resolveWithOpts` seam carries strictly-validated per-pack `opts:` from `graders.yaml`; plugin graders receive the record **untouched and unvalidated** as an optional third argument (`PluginGraderWithOpts`). | EVAL | T1, T8 | `eval-grader`, `eval-judge`, `model-adapter` |
| ✅ `feedback-distill` | The rating→dataset pipeline as a package: `FeedbackRecord` + turn derivation, multi-rater resolution (majority / mean / adjudication, with a true tie withheld and queued), `distill()` + grader synthesis, the shared PII/secret redactor, split + version helpers, the auto-distill watermark and daemon janitor step, and the append-only human-review queue (`.crewhaus/review/queue.jsonl`). Brief: 300. | EVAL, CLI, CHN, MGD | T1 | `eval-dataset`, `eval-grader`, `dataset-registry`, `event-log` |
...
| ✅ `eval-optimizer-orchestrator` | Pillar 2 orchestration — wires `eval-runner` (fitness) + `prompt-optimizer` (search) + `spec-patch` (mutation) into the `crewhaus optimize` active loop. Now drives **multi-stage** specs (workflow / graph / crew / pipeline) through the same `compile --with-eval-harness` emission and bridge invoker, one gated stage at a time (`promptPath` option, `--stage` flag), and emits one whitelist-validated `SpecPatch` per moved numeric dial into `patches.json`. | EVAL, CLI | T1, T3 | `eval-runner`, `prompt-optimizer`, `spec-patch`, `target-eval-bundle` |
| ✅ `meta-harness-optimizer` | Experimental §56 mutator: a filesystem-backed **experience store** of every measured candidate (artifact, per-sample scores, trace) that the `--mutator meta-harness` proposer reads instead of a summary window. Same accept gate, budget meter and `OPTIMIZABLE_PATHS` validation; the CLI path is spec-shaped (replacement instructions that round-trip through `parseSpec`) and the bundle-rewriting mode stays library-only. | EVAL, CLI | T1, T3 | `prompt-optimizer`, `model-router` |
...
| ✅ Production graders | `grader-nlg-metrics` / `-semantic-similarity` / `-safety-classifiers` / `-multimodal`. Reachable from `graders.yaml` under namespaced registry names with validated `opts:`. `safety.toxicity` / `safety.bias` now resolve a judge-backed classifier lazily from `CREWHAUS_EVAL_CLASSIFIER` or `opts.classifier`, with `safety.toxicity.heuristic` / `safety.bias.heuristic` as the honest offline keyword mocks (never a silent default for the real names); `multimodal.imageOcrThenGrade` resolves OCR from `CREWHAUS_EVAL_VISION_MODEL` or `opts.model` and grades the text with `opts.textGrader`. `semantic.similarity`'s ROUGE-L degradation is now reported at run level (`aggregates.semanticFallback` + a stderr warning) and disableable from YAML. **`multimodal.audioTranscriptMatch` remains a wiring-explaining thrower** — no bundled adapter carries audio input, so a plugin/programmatic `SttFn` is the only route. | EVAL | T1, T8, T9 | `grader-registry`, `embedder` |
| ✅ Runner-local grader packs | `calibration.abstentionAware` (answered-correct / answered-wrong / **not-attempted** classification, emitting `aggregates.calibration` = answer rate, abstention rate, accuracy-when-answered) and `consistency.paraphraseGroup` (a per-sample **vacuous pass** whose real work is at aggregation: verdict consistency within a `metadata.paraphrase_group`, emitting `aggregates.paraphraseConsistency`). Declaring the latter shifts `meanScore` upward by construction — `passRate` under `all`/`any` is unaffected. | EVAL | T1 | `eval-runner`, `eval-grader` |
...
| **Eval Stack** | eval-dataset · eval-runner · eval-grader · eval-judge · eval-report · dataset-registry · grader-registry · feedback-distill · regression-runner · prompt-optimizer · replay-engine | EVAL target or CI for any target |
