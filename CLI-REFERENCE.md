...
>
> **v0.6.0** makes per-model settings a compiled fact: a top-level
> `models:` registry, `$profile` references at every model slot, and hybrid
> setups (cascade, guide, shadow, committee, `Consult`/`Escalate`). The CLI
> half is `models list|explain|audit|propose`, a rebuilt `route` namespace
> (`explain --json`, `status --by`, `freeze`, `promote`, `propose`),
> `eval --routing` plus `eval leaderboard`, `judge calibrate --by-model`,
> `upgrade --hoist-models`, and `init --hybrid`. Additive as ever: a spec
> that declares no `models:` block and no pool strategy compiles
> byte-identically, and the roster stays human-owned — every automation here
> proposes, and a person merges.
>
...
| `init --hybrid` | Scaffold the **hybrid** spec instead of the single-model one: a small `models:` registry, a two-arm `model_pool`, the cascade (cheap drafts → a stronger checker grades → the strong model redoes it on failure), and the `evaluation.on_fail: escalate` judge that triggers it — every line commented. The fast/strong pair is read from the installed pricing table and is always **within one provider**; a cross-provider pair needs a second credential and a second vendor seeing your transcripts, which is your call rather than a scaffold's. `init --interactive` asks the same question once. |
...
| `upgrade [spec] [--write]` | Detect the spec's `version:` drift vs the current CLI and run the migration chain (each step re-validated through `parseSpec` before anything is written). Dry-run diff by default; `--write` applies, then registers the new version + changelog entry like `compile` does. Since v0.6.0 the migration is **comment- and key-order-preserving** — each step is a list of CST edits rather than a re-serialised object — and the command also prints the per-release upgrade notes that apply to this spec. |
| `upgrade … --hoist-models [--rewrite-arms]` | Lift `{model, thinking, max_tokens}` triples repeated on two or more slots into `models:` profiles named by price rank and rewrite those slots to `$refs`. A proposal by default — it prints the diff, and the lowered IR is identical either way. `--rewrite-arms` additionally re-keys the learned scoreboard for candidates that became profiles, so the history survives the hoist instead of being orphaned; it is coverage-gated (a model split across two profiles cannot be attributed line by line and is reported as a reset) and requires `--write`. |
| `migrate-all --from <N> --to <N> [--dry-run]` | Batch-migrate every spec in the registry across **spec-schema** versions — the `version:` field a spec declares, not the IR (the IR is rebuilt from the spec on every compile and has nothing to migrate). Shares `upgrade`'s comment-preserving writer and its spec validation. |
...
| `eval … --models <m1,m2,…>` | Benchmark matrix: run the same dataset+graders once per model; each cell writes to `<out>/<model-slug>/` and the run emits `matrix.json` + `index.html`. Since v0.6.0 the list also accepts **`$profile` refs and the word `pool`** — `--models '$fast,$strong'` runs one cell per `models:` profile, `--models pool` expands the whole `model_pool` roster (quote the refs, or the shell eats them). Adding `--record` runs every cell through the run-history flow under its **own per-arm lineage**, which is what makes `--gate`/`--no-promote` legal with `--models`; without it, cells stay comparisons rather than lineage and those flags are refused. Incompatible with `--max-p95-latency-ms`/`--max-cost-usd`, with `--record-tools`/`--replay-tools` (cells share sample ids), with `--resume`, and with `--routing`. |
| `eval … --routing static\|as-declared\|candidate:<$profile\|model>` | **Route the eval.** Default `static` is the configured `agent.model`, exactly as before. `as-declared` wires the spec's own pool / tiers / fallbacks so you measure what production serves; `candidate:$fast` pins one roster member and measures that arm alone, with *its* request params and instructions overlay. Every routed mode pins the pool's learning seed and routes off a **frozen arm snapshot** — the run reads arm statistics and records none, so a measurement can never move the harness's learned policy, and two runs of the same seed make identical decisions. Routed runs key their own baseline lineage beside the legacy `(spec, dataset)` one, so a cheap candidate can never pin over the primary's baseline. |
| `eval … --warm-arms` | Seed that frozen snapshot from the harness's live `arms.jsonl` instead of starting cold. The file is re-read at the end and a mid-run change is reported. The snapshot's digest is the run's **instrument identity**: two runs with different digests start a new baseline lineage rather than gating across the difference. Needs `--routing`. |
| `eval leaderboard <matrix-dir> [--pairwise] [--export-priors <file> [--spec <s>]] [--min-n N] [--seed N] [--json]` | Rank the arms of a recorded `--models` matrix, entirely offline (it opens run directories and makes no model call, so it is safe in CI). Each row carries a Wilson interval on pass rate and a Student-t interval on mean score; the verdict comes from a **paired** sign-flip permutation test over per-sample deltas, Holm-corrected across comparisons. It **refuses to name a winner the evidence cannot support** — `UNDERPOWERED` below the comparable-pair floor (`--min-n`, default 30), `TIE` when the test is not significant or the top two intervals overlap — and writes that verdict back into `matrix.json` as an additive block. Cells graded with a different graders hash or judge model are excluded rather than ranked beside comparable ones. `--pairwise` prints every head-to-head with raw and adjusted p-values (a statistical comparison of recorded runs — it makes no judge calls; that is `eval-report diff --pairwise`). `--export-priors` writes the measurement as routing priors in reward units, pseudo-count capped so a prior can never outweigh a handful of live observations, and fingerprinted against the spec's roster so a roster edit invalidates it loudly. |
...
| `optimize … --from-advice --routing <mode> [--warm-arms]` | Gate routing suggestions on a **routed** eval — the gate runs on what production serves, not on the static primary — with one frozen arm snapshot shared by every candidate, so two candidates are compared against the same instrument. Takes the same `--routing` / `--warm-arms` values as `eval`. |
...
| `flywheel init --model-plan` | Also scaffold the nightly model-plan job: `models audit --propose` plus `route propose`, opening a PR and never merging one. It runs with `continue-on-error`, so a red audit still produces the replacement PR instead of swallowing it. |
...
| `judge calibrate … --by-model` | Calibrate **one cut per (agent arm, judge model) pair** instead of one for the whole spec. Rated turns are attributed to the arm that served them (from the session's own routing lines — a `models:` profile name, else the model string), so a cheap arm checked by a strong judge gets its own gate rather than inheriting the spec's. With `--graders`, each arm is judged by the judge its `per_model.judge` names (a `$profile` ref resolves through the local spec), else the grader's own `model:`, else `--model` — the same order the eval runner resolves, so the pair keys written here are the ones it looks up. With `--apply` the pairs land under a `byPair` map and the runner resolves pair → spec → default, so unrouted runs are unaffected. Turns with no routing lines fold into the spec-level cut only. |
...
on `type: registry` entries, and — since v0.6.0 — a `per_model:` map that
binds each **served arm** to its own judge, passing cut and weight
(`$fast: { judge: $strong, passing_score: 4 }`). That map is read on a routed
run (`eval --routing`); a static run has no served arm, so it grades with the
base judge and says the map went unused. Both the top level and the
`llm_judge` and `registry` entries are now **strict**, so a key that used to
be silently stripped is a loud parse error. The grammar reference lives in
...
| Command | Purpose |
| --- | --- |
| `advise [--session <id> \| --all] [--json] [-o <dir>]` | Mine session logs + audit records for typed `SpecPatch` suggestions; writes `suggestions.json` + `report.html` (default `.crewhaus/advice`). Feed the result to `optimize --from-advice`. v0.6.0 adds four routing rules — escalation precision and recall (is the cascade firing on the right turns?), audition-ready, and policy-flip-ready. |
...
Provider failover, the model registry, hybrid routing, market scanning,
right-sizing, and budget caps.

**The roster is human-owned.** `agent.model`, `model_pool.candidates`, and a
profile's identity and security surface (`model`, `tags`, `tools`,
`tool_config`, `permissions`, `rate_limits`, `cost`, `requires`,
`capabilities`, `fallbacks`, `circuit_breaker`, `instructions`) all sit
outside `OPTIMIZABLE_PATHS`, so nothing here writes them for you — the write
paths are direct comment-preserving CST edits, always human-initiated, or a
pull request. What v0.6.0 *did* open to the optimizer is the dial half:
`agent.temperature`, a profile's `max_tokens` / `thinking.budget_tokens` /
`temperature` / `limits.model_call_timeout_ms`, `model_pool.rules[*].enabled`,
and the pure cost dials `strategy.shadow.sample_rate`,
`strategy.guide.max_tokens`, `strategy.max_escalations`,
`strategy.cascade.clean_prompt` and `classifier.max_tokens`. Rule targets,
classifier identity, every `strategy.*` role slot, `strategy.model_directed`,
`reward.*`, `directives` and `scope` stay human-owned, and a path containing
the segment `model_pool` is now optimizable **only by exact match** — the
whole-block `steps` / `nodes` / `roles` entries can no longer reach into a
pool by prefix.

| Command | Purpose |
| --- | --- |
| `models list [<spec>]` | Print the resolved `models:` profile registry — one row per profile, with the model each `$ref` resolves to. |
| `models explain [<spec>]` | Every model slot's resolved profile, the hybrid strategy in one sentence, how `/model` directives are handled, and the per-shape carry/emit/ignore verdict for this spec's own shape (the same table published in [COMPILER-ARCHITECTURE](COMPILER-ARCHITECTURE.md#per-shape-carry--emit--ignore)). |
| `models audit [<spec>] [--fail-on none\|pricing\|sunset] [--today YYYY-MM-DD] [--propose] [--json]` | Walk every slot — profiles, agent, steps/nodes/roles, pool candidates, tiers, fallbacks, judges, sub-agents — and check three things: pricing coverage, a declared `requires:` against the capability table, and **per-(provider, knob) parameter acceptance**, projected through each adapter's own marshaller so a silently-dropped `temperature` is visible offline (see [PROVIDERS](PROVIDERS.md#which-knobs-each-route-accepts)). `--fail-on` is a ladder: `none` never exits non-zero, `pricing` (default) fails on breakage that is true today — a pricing miss, an unsatisfiable `requires`, a rejected parameter, or a compiled-in sunset already past `--today` — and `sunset` additionally fails on an announced, not-yet-past one. Sunsets installed from a fetched pricing feed stay advisory at every level, so syncing a feed can never flip an exit code. `--today` pins the clock for tests. |
| `models propose [<spec>] --source sunset\|audition [-o <dir>]` | Write a review bundle, never an edit. `--source sunset` proposes the replacement for each retired slot (the same bundle `models audit --propose` writes) and prints the exact `model right-size` command that gates it. `--source audition` proposes a roster change for a shadow-lane arm that has cleared the power floor and beats the live arms; below the floor it refuses loudly with the count, because proposing a roster change on a dozen observations is the failure the floor exists to prevent. |
...
| `route status [--by profile\|scope] [--shadow] [--json] [--dir <root>]` | Show the adaptive `agent.model_pool` reward scoreboard: per-band arms with sample count, mean reward, mean latency/cost, best-per-band starred (what a `learned` policy exploits). `--by` regroups the table by profile or by routing scope; `--shadow` reveals the observe-only lanes, which are hidden by default because they steer nothing. |
| `route explain <session> [--json] [--dir <root>]` | Replay one run as a **timeline**: the routing decisions, the `/model` directives a typed input seam parsed, and the hybrid stage transitions, interleaved in turn order — so the shape of a hybrid turn (draft → judge → escalate) reads back as itself rather than as a flat list of model calls. `--json` renders every durable field verbatim (hint, eligibility, rule id, classifier verdict, policy, profile, arm, scope, floor, back-off), so a reviewer reads the decision rather than a rendering of it. |
| `route propose [--json] [-o <dir>] [--dir <root>]` | Mine the scoreboard and the installed priors into `SpecPatch`es on **whitelisted paths only**, written as the same `suggestions.json` `crewhaus advise` emits — so `optimize --from-advice` eval-gates them before anything is written back. The roster itself is never proposed; that is `models propose`'s door. |
| `route freeze <policyVersion> [--reason <text>]` / `route freeze --clear` | Pin the learned policy. While the marker exists, every pooled run in that root routes off the frozen history, records no new observation, and reports the frozen policy version on its routing lines. The second kill switch beside `reset`: `freeze` stops learning without losing it. |
| `route promote [--gate] [--spec <n>] [--dry-run] [--json]` | Fold the observe-only quality and shadow lanes into live arms — the one sanctioned path out of "observed but never acted on", and a deliberate, eval-gated, audited act rather than a background drift. `--gate` maps a refused promotion to a non-zero exit for CI; `--dry-run` reports the fold without writing. Recorded as a `routing_promotion` audit entry. |
| `route reset [--dir <root>]` | Wipe the `model_pool` reward scoreboard, and the freeze marker with it (a reset is "start over"). `--dir` points at the `.crewhaus` root (default `.crewhaus`). |

Model automation is also declarative in the spec: the top-level `models:`
registry and `$profile` references at every model slot (v0.6.0);
...
exclusive with the previous two), the pool's `rules` / `classifier` /
`strategy` / `reward` blocks that make hybrid setups expressible, and
`budget:` (run-level spend cap, now with a `judge_share` sub-cap so the
checker's spend is inside the ceiling rather than beside it). Adaptive
routing also applies to the interpreted `crewhaus run` path, not just compiled
bundles. See [Per-model settings and hybrid setups](GETTING-STARTED.md#per-model-settings-and-hybrid-setups)
and the [spec reference](GETTING-STARTED.md#model-cost-and-budget-blocks).
...
| `deploy canary … --routing-gate` | Swap in the routing-aware regression gate: escalation rate, cost per turn and quality-floor blocks are gated alongside pass rate and p95, so a routing change is judged on the axes it actually moves. Run the eval `--routing as-declared` (or supply the routing deltas) for the comparison to be meaningful. |
...
| `services setup <spec> [--zone <domain>] [--port <n>] [--services slack,cloudflare,thredz,agentmail] [--hostname <fqdn>] [--tunnel <name>] [--space-type shared\|individual] [--app-id <id>] [--manual-install] [--inbox-var <VAR>] [--mail-domain <d>] [--scoped-key <VAR>] [--env-file <path>] [--dry-run] [-y]` | Create the external services the spec asks for: a Slack app (manifest applied, both request URLs set, then installed), a Cloudflare named tunnel with a public hostname and DNS, a Thredz wiki space, and an AgentMail inbox. Credentials land in the harness `.env`; the wiki slug lands in the spec. Provisioning tokens are read once and never stored — including AgentMail's org key, which creates the inbox but never enters the harness (`--scoped-key` mints one scoped to that inbox instead). |
