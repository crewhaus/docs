...
| `eval <spec> --dataset <d> --graders <g>` | Run the agent against a dataset and grade it (deterministic graders + LLM-as-judge). `--dataset` also accepts `registry:<ref>`. Flags: `--judge-model`, `--concurrency`, `--seed`, `-o`. |
