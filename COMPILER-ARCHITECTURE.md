...
**Where this table lives in code.** The hybrid columns are machine-readable: `HYBRID_FAMILIES_BY_SHAPE` in [packages/model-service/src/index.ts](https://github.com/crewhaus/factory/blob/main/packages/model-service/src/index.ts) maps each shape to the closure families its compiled bundle constructs — `modelDirected` (Consult + Escalate), `classifier` (the `policy: classifier` label call) and `sideCalls` (guide / shadow / committee). The emitters read it, and so does the compiler's `model-plan-ignored-on-shape` warning ([packages/compiler/src/index.ts](https://github.com/crewhaus/factory/blob/main/packages/compiler/src/index.ts)) — so for the three hybrid columns, the matrix above and the bundles cannot drift apart.

The other nine columns are not pinned by that table. `crewhaus models explain` prints a shape's full row from `SHAPE_MODEL_MATRIX` in [apps/cli/src/models-cli.ts](https://github.com/crewhaus/factory/blob/main/apps/cli/src/models-cli.ts) — a second, hand-maintained transcription of the whole matrix, kept in step with this page by review rather than by a test. If you change a row here, change it there too. Reading the families table against the matrix:
